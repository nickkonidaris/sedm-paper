...
\begin{abstract}
The Spectral Energy Distribution (SED) Machine, for the Palomar 60-inch telescope, is a back-focus mount integral-field spectrograph design to take thousands of classification-quality spectra per on robotic telescopes less than 3-m in diameter. To date, we have been able to classify sources down to $r\sim20$ utilizing the spectrographs 0.36 $\mu m$ to 1.0 $\mu m$ wavelength range. In this paper we present the design and construction of the SED Machine for the robotic Palomar 60-inch telescope (P60). On P60 SED machine is a ugri imager with a 12x12 minute field and a spectrograph with resolution ($R=\lambda/\Delta\lambda$) 100 covering the wavelength range from 370 nm - 1,000 nm, fed by an integral-field unit with a 25$\times$25 arcsecond field of view. 
\end{abstract}

\keywords{}
\section{Introduction}

In 2009, at the beginning of the Palomar Transient Factory (PTF) survey, we recognized that a large amount of observing time from our ``big glass'' (the 200-inch Hale, Gemini 8-m, and the W.M. Keck 1 and 2 telescopes) was allocated to classifying sources. By using big glass we were able to achieve the required signal to noises within minutes, but the classification cadence was limited by the time-allocation process to a night every few weeks. For a survey like PTF with a few-day cadence, our followup cadence and discovery cadence were mismatched. 

During a planning retreat, the authors and Shri Kulkarni recognized that by allocating a substantial amount of time on a smaller telescope with an efficient spectrograph, we could meet the discovery cadence. Palomar observatory's 60-inch telescope was the prime candidate for such a classification spectrograph.

At the same time, simply scaling the aperture areas of the 10-m telescope to the 60-inch, the 60-inch has at least 40x less light-collecting area for the point sources we care about. Given that spectrographs on large telescopes operate near the best efficiency afforded by current coating, grating, and detector technology, the main knob we could turn to improve classification efficiency was designing a spectrograph with {\em the minimum useful} spectral resolution. In addition, to maximize classification efficiency, we operate over the broadest wavelength range supported by our detectors, use an integral field unit to minimize acquisition time, and employee a second calibration camera.

The SED Machine was funded by the United States National Science Foundation\footnote{NSF Number Here} in the summer of 2010, two years later we achieved first light. At first light, we discovered that the throughput of SED Machine in the blue is about half of what we expected, and that our data reduction pipeline would not deliver correct spectra for objects fainter than about $r\sim14$. For the past several years, we worked on improving the data reduction pipeline. The new pipeline is able to extract spectra down to $r\sim20.5$ \citep[see forthcoming work][]{neil2016}.

This paper describes the design and construction of SED Machine's hardware. We have organized the paper as follows, in \S\ref{sec:metric} the optimization strategy of SED Machine is discussed, based on the optimization strategy we describe the as-designed hardware in \S\ref{sec:design}
...
A collimator, prism, and camera follows the front end and focuses the spectra onto a Princeton Instruments PIXIS eXcelon detector. The camera sits on a flexure that is pushed with a linear Newport stage, the stage allows spectrograph focus to be adjusted.
...
\bibitem[Bacon et al.(2001)]{bacon2001} Bacon, R., Copin, Y., Monnet, G., Miller, B. W., Allington-Smith, J. R., Bureau, M., et al. (2001). The SAURON project - I. The panoramic integral-field spectrograph. Monthly Notices of the Royal Astronomical Society, 326(1), 23Ð35. doi:10.1046/j.1365-8711.2001.04612.x
...
\bibitem[Neil et al.(2016)]{neil2016} Neil, D., Konidaris, N., Barlow, T., etc.... SED Machine Data Reduction Pipeline. {\em to be submitted}.

\bibitem[Sugai et al.(2010)]{sugai2010} Sugai, H., Hattori, T., Kawai, A., Ozaki, S., Hayashi, T., Ishigaki, T., et al. (2010). The Kyoto Tridimensional Spectrograph II on Subaru and the University of Hawaii 88Êin Telescopes. Publications of the Astronomical Society of the Pacific, 122(887), 103Ð118. doi:10.1086/650397
