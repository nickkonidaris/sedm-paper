...
The SED Machine is designed to  classifications per unit time. 

The main functional specifications of the spectrograph are as follows. Spectral resolution,  field of view, and wavelength range drive the cost and complexity of spectrographs. 

For the SED Machine whose goal is to identify supernovae spectral resolution 100 was identified as a sufficient resolution for such typing (and is demonstrated in spectra shown in the appendix). Such resolution is much lower than what is typical considered low resolution. Working at $R\sim100$ allowed us to use a prism which afforded a factor of between 10\% and 50\% more throughput than a diffraction grating.
...
To maximize the ability to type, the wavelength range should cover from the emission of [O II] (370 nm) to typically redshifted Ca II (920 nm). At the same time, the available packaged commercial charge-coupled detectors (CCD) had acceptable throughput over the 370-920 nm range. This wavelength range is also comfortable for state-of-the-art anti-reflection coatings for lenses that work well over a factor of 2.4$\times$. 

Throughput is of high concern for any instrument. Aside from the above considerations, we deemed that throughput over the broad wavelength range should be greater than what is typical of modern single slit spectrographs ($>35\%$). 
